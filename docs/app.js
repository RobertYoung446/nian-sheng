...
function updateChrome() {
  ...
  document.querySelector("#weekCount").innerHTML =
    `${Math.min(count, 7)} <span>个想法</span>`;
  document.querySelector("#streakBars").innerHTML = [0, 1, 2, 3, 4, 5, 6]
    .map((i) => `<i class="${i < Math.min(count, 7) ? "lit" : ""}"></i>`)
    .join("");
  document
...
function renderReview() {
  const list = shownIdeas();
  const average = (key) =>
    Math.round(list.reduce((sum, item) => sum + item[key], 0) / list.length);
  const clarity = average("clarity");
  view.innerHTML = `<section class="review-hero glass"><div><span class="tag">WEEKLY REVIEW</span><h2>你不需要做完所有想法<br>只需要看清真正重要的。</h2><p>这是根据当前浏览器里的记录生成的复盘。</p></div><div class="review-ring" style="--score:${clarity * 3.6}deg"><strong>${clarity}</strong><small>方向清晰度</small></div></section><div class="review-stats">${[
    ["本周新增", ideas.length, "个真实想法"],
...
async function openOcean() {
  ...
  const points = layoutPoints(list);
  ...
  try {
    ...
    status.classList.add(ai.apiKey ? "model" : "");
